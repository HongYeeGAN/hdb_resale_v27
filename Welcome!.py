...
st.markdown('<style> .mleft { margin-left: 50px; } </style>', unsafe_allow_html=True)
st.markdown(
    """
    <div style='padding: 3px;class="mleft"'>
...
st.markdown(
    """
    <div padding: 0px'>
        <p style='color: #808080;'>Click on the bubble to select town<br>
        Color represent price per square meter    •      Size represent transaction count</p>
    </div>
    """,
    unsafe_allow_html=True
)
...
figure = px.scatter(map4, x='Price/sqm (S$)', y='Distance from city center in km', animation_frame="year",
                    ...
                    log_x=False, size_max=50, text='town', opacity = 0.6,
                    title =' ',
...
figure.update_layout(width=700, height=800, title_font_color='#808080',
                    plot_bgcolor="white", title_font=dict(size=1),
                    ...
                    "transition": {"duration": 0, "easing": "linear"}}])], x=0.05, y=1.25, active=0)],
                     sliders=[dict(active=0, visible=True, x=0.05, y=1.25, len=1)],
                     )
